...
if not DB_PASSWORD:
    raise RuntimeError(
        "Set PAID_MEDIA_DB_PASSWORD before loading data into PostgreSQL."
    )

safe_password = urllib.parse.quote_plus(DB_PASSWORD)

DB_CONNECTION_URL = (
    f"postgresql://{DB_USER}:{safe_password}"
...
def load_data_to_postgres():
    ...
    with engine.begin() as connection:
        connection.execute(
            text("TRUNCATE fact_ads_performance RESTART IDENTITY")
        )
    ...
    for table, (column, values) in dimensions.items():
        ...
        new_values = pd.DataFrame(
            {column: values.unique()}
        )

        new_values = new_values[
            ~new_values[column].isin(current)
        ]
    ...
    platform = pd.read_sql(
        "SELECT platform_id, platform_name FROM dim_platform",
        engine,
    )

    campaign = pd.read_sql(
        "SELECT campaign_type_id, campaign_type_name "
        "FROM dim_campaign_type",
        engine,
    )

    industry = pd.read_sql(
        "SELECT industry_id, industry_name FROM dim_industry",
        engine,
    )
